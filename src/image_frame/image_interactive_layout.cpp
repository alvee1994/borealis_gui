...
void ImageInteractiveLayout::setConnections(){
  ...
  connect(this->save_as_image, SIGNAL(pressed()), this, SLOT(zoomInImage()));
...
void ImageInteractiveLayout::setImageFrame(){
  image_frame = new borealis_gui::RatioLayoutedFrame(scroll_area_widget_contents);
...
void ImageInteractiveLayout::zoomInImage()
{
  if (this->image_frame->getImage().isNull())
  {
    return;
  }
  this->image_frame->setInnerFrameFixedSize(this->image_frame->getImage().size());
}
